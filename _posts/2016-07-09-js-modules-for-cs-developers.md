...
JavaScript is a rather odd language and the ecosystem is rather huge, as with the .Net environment.  There are just *so* many choices.  As a result I have largely ignored anything that did not directly relate to my work.  As such I ended missing the entire JavaScript ***module*** train.  Now for the upcoming EcmaScript 6 [JavaScript Modules](http://exploringjs.com/es6/ch_modules.html) are pretty well defined.  Now, if you are familiar with JavaScript modules you may as well ignore this post.
